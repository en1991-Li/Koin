...
const CATEGORY_DATA = {
    expense: [
        ...
        { id: 'entertainment', name: '娛樂', icon: 'gamepad-2' },
        { id: 'medical', name: '醫療', icon: 'heart-pulse' }
    ],
    income: [
        { id: 'salary', name: '薪資', icon: 'wallet' },
        { id: 'bonus', name: '獎金', icon: 'award' },
        { id: 'investment', name: '投資', icon: 'trending-up' },
        { id: 'side-hustle', name: '副業', icon: 'laptop' },
        { id: 'allowance', name: '零用錢', icon: 'coins' },
        { id: 'other', name: '其他收入', icon: 'badge-dollar-sign' }
    ]
};

/**
 * 2. 核心切換函式：點擊 支出/收入 標籤時觸發
 */
function switchRecordType(type) {
    // A. 更新全域記帳狀態中的類型
    recordState.type = type; 
    
    // B. 切換畫面上「支出/收入」標籤的 active 樣式 (黃底黑字切換)
    document.getElementById('tab-record-expense').classList.toggle('active', type === 'expense');
    document.getElementById('tab-record-income').classList.toggle('active', type === 'income');
...
function renderRecordCategories() {
    const gridContainer = document.getElementById('record-category-grid');
    if (!gridContainer) return;
    
    gridContainer.innerHTML = ''; // 先清空舊的分類
    
    // 根據當前狀態 (expense 或 income) 抓取對應的陣列
    const currentType = recordState.type || 'expense';
    const categories = CATEGORY_DATA[currentType];
    
    // 跑迴圈動態生成每一個分類的 HTML 按鈕
    categories.forEach((cat, index) => {
        // 預設選中第一個分類
        const isSelected = index === 0;
        if (isSelected) recordState.category = cat.id; // 同步寫入狀態
        
        const activeClass = isSelected ? 'active' : '';
        
        const itemHTML = `
            <div class="category-item ${activeClass}" id="cat-${cat.id}" onclick="selectRecordCategory('${cat.id}')">
 ...
 * 4. 點擊特定分類圖標時的選取高亮邏輯
 */
function selectRecordCategory(catId) {
    recordState.category = catId;
    
    // 移除畫面上所有分類的 active 樣式，並只加在當前點擊的分類上
    document.querySelectorAll('.category-item').forEach(el => el.classList.remove('active'));
    
    const currentTarget = document.getElementById(`cat-${catId}`);
